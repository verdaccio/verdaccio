...
import configDefault from '../partials/config/config_urlbase';
...
const basePath = '/basepathtest';
const urlPrefix = '/urlprefixtest/';
describe('endpoint unit test', () => {
  ...
  beforeAll(function(done) {
    const store = path.join(__dirname, '../partials/store/test-storage-urlbase');
    const mockServerPort = 55555;
    rimraf(store, async () => {
      const configForTest = _.clone(configDefault);
      configForTest.auth = {
        htpasswd: {
          file: './test-storage-urlbase/.htpasswd'
        }
      };
      ...
      configForTest.base_path = `${basePath}/`;
      configForTest.url_prefix = urlPrefix;
  ...
  describe('Registry API Endpoints with an URL Prefix and a Base Path', () => {
    ...
    describe('should test user api', () => {
      ...
      test('should test add a new user', (done) => {
        request(app)
          .put(`${basePath}/-/user/org.couchdb.user:jota`)
          .send(credentials)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
      ...
      test('should test add a new user with login', (done) => {
        ...
        request(app)
          .put(`${basePath}/-/user/org.couchdb.user:jotaNew`)
          .send(newCredentials)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .expect(HTTP_STATUS.CREATED)
          .end(function(err, res) {
    ...
    describe('should test package api', () => {
      ...
      test('should fails on fetch jquery specific tag package from remote uplink', (done) => {
        ...
        request(app)
          .get(`${basePath}/jquery/never-will-exist-this-tag`)
          .set(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
      ...
      test('should not found a unexisting remote package under scope', (done) => {
        ...
        request(app)
          .get(`${basePath}/@verdaccio/not-found`)
          .set(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .expect(HTTP_STATUS.NOT_FOUND)
          .end(function(err, res) {
      ...
      test('should fetch a tarball from remote uplink', (done) => {
        ...
        request(app)
          .get(`${basePath}/jquery/-/jquery-1.5.1.tgz`)
      ...
      test('should fails fetch a tarball from remote uplink', (done) => {
        ...
        request(app)
          .get(`${basePath}/jquery/-/jquery-0.0.1.tgz`)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.OCTET_STREAM)
          .expect(HTTP_STATUS.NOT_FOUND)
          .end(function(err) {
            if (err) {
    ...
    describe('should test dist-tag api', () => {
      ...
      test('should set a new tag on jquery', (done) => {
        ...
        request(app)
          .put(`${basePath}/jquery/verdaccio-tag`)
      ...
      test('should fetch all tag for jquery', (done) => {
        ...
        request(app)
          .get(`${basePath}/-/package/jquery/dist-tags`)
          .set('accept-encoding', HEADERS.JSON)
          .set(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON)
          .expect(HTTP_STATUS.OK)
          .end(function(err, res) {
      ...
      test('should update a new tag on jquery', (done) => {
        ...
        request(app)
          .post(`${basePath}/-/package/jquery/dist-tags`)
      ...
      test('should set a remove a tag on jquery', (done) => {
        ...
        request(app)
          .del(`${basePath}/-/package/jquery/dist-tags/verdaccio-tag`)
          .set('accept-encoding', HEADERS.JSON)
          .set(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON)
          //.expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .expect(HTTP_STATUS.CREATED)
    ...
    describe('should test search api', () => {
      test('should perform a search', (done) => {
        ...
        request(app)
          .get(`${basePath}/-/all/since?stale=update_after&startkey=${cacheTime}`)
          // .set('accept-encoding', HEADERS.JSON)
          // .set(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON)
          //.expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .expect(HTTP_STATUS.OK)
    ...
    describe('should test publish api', () => {
      ...
      test('should unpublish a new package', (done) => {
        ...
        request(app)
          .del(`${basePath}/@scope%2fpk1-test/-rev/4-6abcdb4efd41a576`)
          .set(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON)
          .expect(HTTP_STATUS.CREATED)
          .end(function(err, res) {
            if (err) {
  ...
  describe('Registry WebUI endpoints', () => {
    ...
    describe('Packages', () => {
      ...
      test('should display all packages', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/packages`)
          .expect(HTTP_STATUS.OK)
          .end(function(err, res) {
      ...
      test.skip('should display scoped readme', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/package/readme/@scope/pk1-test`)
          .expect(HTTP_STATUS.OK)
      ...
      test.skip('should display scoped readme 404', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/package/readme/@scope/404`)
          .expect(HTTP_STATUS.OK)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.TEXT_CHARSET)
          .end(function(err, res) {
      ...
      test('should display sidebar info', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/sidebar/@scope/pk1-test`)
          .expect(HTTP_STATUS.OK)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .end(function(err, res) {
      ...
      test('should display sidebar info 404', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/sidebar/@scope/404`)
          .expect(HTTP_STATUS.NOT_FOUND)
          .expect(HEADER_TYPE.CONTENT_TYPE, HEADERS.JSON_CHARSET)
          .end(function() {
            done();
    ...
    describe('Search', () => {
      ...
      test('should search pk1-test', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/search/scope`)
          .expect(HTTP_STATUS.OK)
          .end(function(err, res) {
            expect(res.body).toHaveLength(1);
            done();
      ...
      test('should not find forbidden-place', (done) => {
        request(app)
          .get(`${basePath}/-/verdaccio/search/forbidden-place`)
          .expect(HTTP_STATUS.OK)
          .end(function(err, res) {
