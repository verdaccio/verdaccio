...
describe('StorageTest', () => {
  ...
  describe('getTarball', () => {
    ...
    test('should not fetch off-uplink tarball for a locally published package', async () => {
      const storage: any = await generateStorage();
      const pkgName = 'ssrf-local-pkg';
      const filename = `${pkgName}-1.0.0.tgz`;
      ensureLocalMetadata(pkgName);
      const pkgDir = path.join(storagePath, pkgName);
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        stream.on('data', () => {
          reject(new Error('Expected a 404 rather than fetching the off-uplink URL'));
        });
        stream.on('end', () => {
          reject(new Error('Expected a 404 rather than fetching the off-uplink URL'));
        });
    ...
    test('should serve CDN tarball via autogenerated proxy for uplink-synced packages', async () => {
      const storage: any = await generateStorage();
      const tarballFixture = path.join(__dirname, '../uplinks/__fixtures__/jquery-1.5.1.tgz');
      const tarballSize = fs.statSync(tarballFixture).size;
      const cdnUrl = 'http://cdn.example.test';

      const origGetPkgMeta = storage.localStorage.getPackageMetadata.bind(storage.localStorage);
      storage.localStorage.getPackageMetadata = vi.fn((name, cb) => {
        if (name === 'jquery') {
          const metadata = JSON.parse(JSON.stringify(jqueryMetadata));
          metadata._uplinks = { npmjs: { etag: '"x"', fetched: Date.now() } };
          metadata._distfiles = {
            'jquery-1.5.1.tgz': {
              url: `${cdnUrl}/jquery/-/jquery-1.5.1.tgz`,
              sha: '2ae2d661e906c1a01e044a71bb5b2743942183e5',
            },
          };
          return cb(null, metadata);
      ...
      nock(cdnUrl).get('/jquery/-/jquery-1.5.1.tgz').replyWithFile(200, tarballFixture, {
        'Content-Type': 'application/octet-stream',
        'Content-Length': tarballSize.toString(),
      });
  ...
  describe('getTarball uplink selection', () => {
    ...
    test('should not attach uplink Authorization when fetching a CDN tarball', async () => {
      const storageConfig = configExample(
        {
          self_path: __dirname,
          storage: storagePath,
      ...
      await fetchTarball(storage, pkgName, filename);
      // off-host CDN tarballs are fetched without the uplink Authorization
      // header (the previous single-uplink fallback leaked that token)
      expect(capturedAuth).toBeUndefined();
    });
