...
start a registry (`pnpm start`, or `node bin/verdaccio --config <yaml> --listen 4873`),
...
On this branch, first ask whether the request is really a `src/` change at all. `src/`
is ~650 lines, and most defects reported against 7.x are already fixed on `master` and
just haven't been bumped here yet — check the pinned `@verdaccio/*` versions in
`package.json` against the versions published on `master`'s dist-tag before writing any
code. If the fix already exists there, the task is running the
`update verdaccio 7.x dependencies` workflow (or opening the equivalent dependency-bump
PR by hand), not implementing anything in `src/`.

Decide which release lines the work concerns:

- A **feature** lands on `master` only; this branch does not take features.
- A **bug fix**: This checkout is branch `7.x`: the `verdaccio` binary in `src/`, on top of published `@verdaccio/*` modules from `master`. A bug in `src/` is fixed here; a bug in a `@verdaccio/*` module is fixed on `master` and bumped here. Check `6.x` for the same defect. A bug fixed here that also exists on another line is not finished until that line has its own PR: `6.x` for the equivalent code path, and `master` when the defect is in a `@verdaccio/*` module rather than in `src/` (then the fix is made there first and bumped here). Say in the PR when a port is pending.
...
config, auth and storage composition, the legacy storage adapter); the published
