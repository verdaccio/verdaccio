...
- `7.x` / `9.x` (`next-7`, `next-9`, `nightly-master` Docker tags) run the
  internals from `master`, which is the checkout you are in.
...
- whether the described behaviour exists today on `master`, and whether the
  same code path exists on `6.x`/`8.x`;
