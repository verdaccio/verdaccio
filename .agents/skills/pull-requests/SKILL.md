...
1. Branch from the current base: `git fetch origin 6.x` then
   `git switch -c <type>/<short-name> origin/6.x`.
2. Run the checks that cover the change with the
   [testing-changes](../testing-changes/SKILL.md) skill, then `yarn lint`,
   `yarn format:check`, and `yarn type-check`. Parts of CI skip draft PRs, so this local pass is
   the only gate until the PR is marked ready.
3. Add one changeset for the PR, naming every published package it touches
   (`yarn changeset` or a hand-written `.changeset/<slug>.md`), when a
...
4. Review your own diff with the [review-code](../review-code/SKILL.md) skill. A
   ...
   round.
5. There are no git hooks on this branch: the lint, format and test pass above is
   the only local gate. Commit normally.
...
```bash
gh pr create --repo verdaccio/verdaccio --base 6.x --title "fix(6.x): <what changed>" --body-file <body.md> [--draft]
```
...
commit, so write the history entry you want. Say which line the PR targets when the same fix exists elsewhere:
...
one release-line label (`6.x branch (latest)` on this branch) plus one to three content labels. The [pr-labels](../pr-labels/SKILL.md) skill
...
5. **Check mergeability** (`gh pr view <n> --json mergeable,mergeStateStatus`).
   ...
   resolved by `yarn install`), then force-push with lease. Re-read the diff after a
...
selection, fix the cause. An e2e failure in one client is a compatibility finding, not noise. The changeset-check job fails on a PR
without a changeset unless a maintainer has labelled it `skip changeset`.

## Ports to other release lines

A bug fixed here that also exists on another line is not finished until that line has its own PR: `master` for the equivalent code path, and `8.x` when the defect is in a `@verdaccio/*` module rather than in `src/` (then the fix is made there first and bumped here). Cherry-pick
onto a branch from that line, adapt (each branch has its own toolchain; code may
