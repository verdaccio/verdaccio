...
1. Branch from the current base: `git fetch origin 7.x` then
   `git switch -c <type>/<short-name> origin/7.x`.
2. Run the checks that cover the change with the
   [testing-changes](../testing-changes/SKILL.md) skill, then `pnpm lint`,
   `pnpm format:check`, and `pnpm type-check`. Parts of CI skip draft PRs, so this local pass is
...
```bash
gh pr create --repo verdaccio/verdaccio --base 7.x --title "fix(7.x): <what changed>" --body-file <body.md> [--draft]
```
...
`fix(7.x): ...` or the same title suffixed `(7.x)`.
...
one release-line label (`7.x branch (next)` on this branch) plus one to three content labels. The [pr-labels](../pr-labels/SKILL.md) skill
...
selection, fix the cause. An e2e failure in one client is a compatibility finding, not noise. The changeset workflow expects a changeset on
every PR that changes the package; a maintainer labels docs and tooling PRs `skip changeset`.

## Ports to other release lines

A bug fixed here that also exists on another line is not finished until that line has its own PR: `6.x` for the equivalent code path, and `master` when the defect is in a `@verdaccio/*` module rather than in `src/` (then the fix is made there first and bumped here). Cherry-pick
