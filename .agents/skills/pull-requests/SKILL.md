...
1. Branch from the current base: `git fetch origin 8.x` then
   `git switch -c <type>/<short-name> origin/8.x`.
2. Run the checks that cover the change with the
   [testing-changes](../testing-changes/SKILL.md) skill, then `pnpm lint`,
   `pnpm format:check`. Parts of CI skip draft PRs, so this local pass is
...
5. Confirm the husky hooks are installed (`git config core.hooksPath` points at
   `.husky/_`); commit normally so the pre-commit format and lint checks run. Never
   `--no-verify`.
...
```bash
gh pr create --repo verdaccio/verdaccio --base 8.x --title "fix(8.x): <what changed>" --body-file <body.md> [--draft]
```
...
`fix(8.x): ...` or the same title suffixed `(8.x)`.
...
A bug fixed here is not finished until it is released (`chore: update versions 8.x`) and bumped on `6.x`, and until `master` has its own PR when the same code path exists there. Features never land here. Cherry-pick
