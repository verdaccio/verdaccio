...
- **Base branch and release line.** The PR must target `7.x`. This checkout is branch `7.x`: the `verdaccio` binary in `src/`, on top of published `@verdaccio/*` modules from `master`. A bug in `src/` is fixed here; a bug in a `@verdaccio/*` module is fixed on `master` and bumped here. Check `6.x` for the same defect. Features belong on `master` only; a feature PR here is declined. For a bug fix, find
...
**First check which shape the PR is** (see AGENTS.md — this branch is ~650 lines of
`src/` and most PRs touch none of it):

- **Dependency bump** (title `chore: update verdaccio 7.x dependencies`, opened by the
  `update-dependencies.yml` workflow): only `package.json` and the lockfile change, and
  the workflow already generated its own changeset. There is no line of code to check
  against the review guide. Instead: confirm every bumped `@verdaccio/*` version is a
  real published version on the dist-tag named in the body (not a hand-edited
  downgrade), confirm the lockfile is regenerated rather than hand-edited, and confirm
  CI (build + the full test suite) is green — that full-suite run is the actual test
  coverage for this PR, since there is no diff to unit-test. Skip the rest of this
  section.
- **Release cut** (title `chore: release 7.x (next-7)`): mechanical `changeset version`
  output — package.json and CHANGELOG bumps, consumed changeset files removed. Verify
  the version bump matches the changesets being consumed and nothing else changed. Skip
  the rest of this section.
- **An actual `src/` change**: apply the full review below.
...
- **Labels**: the release-line label `7.x branch (next)` plus content labels that describe what the PR
