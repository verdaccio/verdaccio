...
On this branch, check the shape first: a PR titled `chore: update verdaccio 7.x
dependencies` or `chore: release 7.x (next-7)` touches only `package.json`, the
lockfile, and a changeset — there is no line of source to run this guide against; see
[review-pr](../review-pr/SKILL.md) for what to check on those instead. Apply the guide
below in full only to a diff that actually touches `src/`.
...
unstaged, and relevant untracked files (`git diff origin/7.x...HEAD`, `git status`).
...
Establish which release lines the change concerns. This checkout is branch `7.x`: the `verdaccio` binary in `src/`, on top of published `@verdaccio/*` modules from `master`. A bug in `src/` is fixed here; a bug in a `@verdaccio/*` module is fixed on `master` and bumped here. Check `6.x` for the same defect. Review with the port in mind (guide §9).
