...
- **Version coverage is part of the fix.** A bug fixed here that also exists on another line is not finished until that line has its own PR: `6.x` for the equivalent code path, and `master` when the defect is in a `@verdaccio/*` module rather than in `src/` (then the fix is made there first and bumped here).
...
The CLI e2e battery (`verdaccio/e2e-tests`) runs npm, pnpm and yarn against this line from
the e2e-tests repository (branch `7.x`). Each client has its own expectations; a change that works for `npm`
alone is not done.
...
- Does it belong on this branch? `7.x` takes bug fixes only; features go to `master`. A fix belongs on every line
  that has the bug.
...
- **Put logic in the owning layer.** This branch owns the wiring in `src/` (startup,
  config, auth and storage composition, the legacy storage adapter); logic that
...
- **No network.** This branch has no network lockdown in the test setup, so a
  reviewer checks that uplinks are mocked; a test that needs the network is wrongly
  designed.
...
- **Change to `src/` or a dependency bump → changeset for `verdaccio` required.** Tests,
  ...
  contributors do not).
- **Bump type**: `patch` for fixes and dependency bumps, `minor` for a capability the 7
  binary gains, `major` only for a deliberate break. `7.x` is in pre mode (`next-7`);
  do not exit it.
...
- New features: `master` only. A feature PR against `7.x` is declined.
- Bug fixes: A bug fixed here that also exists on another line is not finished until that line has its own PR: `6.x` for the equivalent code path, and `master` when the defect is in a `@verdaccio/*` module rather than in `src/` (then the fix is made there first and bumped here).
