...
- **Version coverage is part of the fix.** A bug fixed here is not finished until it is released (`chore: update versions 8.x`) and bumped on `6.x`, and until `master` has its own PR when the same code path exists there. Features never land here.
...
The consumer is the `verdaccio` 6.x binary, exercised by npm, pnpm and yarn in the
e2e battery of `verdaccio/e2e-tests`; `pnpm test:e2e` here covers plugin loading and
storage on disk. Each client has its own expectations; a change that works for `npm`
alone is not done.
...
- Does it belong on this branch? `8.x` takes bug fixes only; features go to `master`. A fix belongs on every line
  that has the bug.
...
- The web UI is `@verdaccio/ui-theme`, developed on `master`; it is not part of this
  branch.
...
- **Put logic in the owning layer.** Identity in `auth`; parsing and defaults in
  `config`; file layout in `local-storage-legacy`; plugin loading in `loaders`; pure
  helpers in `core`. Routing and the store live in the 6.x binary (`src/` on `6.x`).
...
- The **right level**: a unit test in the owning package; the `packages/e2e/*` suites
  for plugin loading and storage on disk; the 6.x Docker image plus the CLI e2e battery
  only for client wiring that a package manager exercises.
...
- **Bump type**: `patch` for fixes and dependency bumps; `minor` only for a backport
  maintainers agreed to ship to 6.x; never `major`. This branch is not in pre mode: a
  `patch` releases to `latest` for 6.x consumers.
- **One changeset per PR, not per package**, listing every published package the PR
  touches with its bump; the fixed
  group (`@verdaccio/core`, `@verdaccio/config`) follows automatically.
- **The text is the changelog entry and must be developed enough**: user-visible effect
  ...
  actually does.
- Breaking changes do not belong on this line; a new config key gets documented in the
  website repository.
...
- New features: `master` only. A feature PR against `8.x` is declined.
- Bug fixes: A bug fixed here is not finished until it is released (`chore: update versions 8.x`) and bumped on `6.x`, and until `master` has its own PR when the same code path exists there. Features never land here.
