...
- **Version coverage is part of the fix.** A security fix on `master` that also affects
  `6.x` is incomplete until the port exists (or is explicitly scheduled).
...
- Does it belong on this branch? Features go to `master`; a fix belongs on every line
  that has the bug.
- Is it a new bundled plugin? Those are accepted only with an accepted discussion thread
  (GitHub Discussions, _Ideas_) linked from the PR; without one, the answer is the
  discussion, not a review.
...
- Web UI changes follow the existing design and i18n path
  (`packages/plugins/ui-theme/src/i18n/crowdin/ui.json`); hard-coded English strings
  in components are a defect.
...
- **Put logic in the owning layer.** Routing and status codes in `api`/`web`;
  merging, filtering, and uplink orchestration in `store`; file layout in
  `local-storage`; HTTP to uplinks in `proxy`; identity in `auth`; parsing and
  defaults in `config`; pure helpers in `core`.
- **Reuse before you write.** `packages/core/core/src/*-utils.ts`,
  `@verdaccio/config`, and `@verdaccio/test-helper` exist for this. Extract to the
...
- The **right level**: a unit test for a helper; a `packages/api` integration test
  (supertest against `initializeServer`) for a route; a `store`/`proxy` test with a
  `nock`ed uplink for merge and proxy behaviour; the e2e CLI battery only for client
  wiring that a package manager exercises.
...
- **No network.** `vitest.setup.mjs` blocks it; a test that needs the network is
  wrongly designed.
- **Conventions**: `test/*.spec.ts`, fixtures from `@verdaccio/test-helper`, no
...
- **Change to a published package → changeset required.** Tests, docs, CI, and
  tooling → none; a maintainer adds the `skip changeset` label (external
  contributors do not).
- **Bump type**: `patch` for fixes, `minor` for user-visible features, `major` for
  breaking config, plugin-interface, or HTTP-contract changes. `master` is in pre mode
  (`next-9`); do not exit it.
- **One changeset per PR, not per package**, listing every published package the PR
  touches with its bump; the fixed
  group (`verdaccio`, `@verdaccio/cli`, `core`, `config`, `node-api`, `ui-theme`)
...
- **The text is the changelog entry and must be developed enough**: user-visible effect
  ...
  actually does.
- Breaking changes and new warnings get an entry in `docs/migrations-guide.md` or
  `docs/warnings.md`; new config keys get documented in the website repository.

---

## 9. Release-line coverage

- New features: `master` only. A `master`-only feature is intentional.
- Bug fixes: determine whether the bug exists on `6.x` (binary) or `8.x` (internal
  modules 6.x uses) as well. If yes, the fix is complete only when a port PR exists or
  the PR description says it is pending.
- The port is reviewed on its own: `6.x` uses yarn and an older toolchain, and the code
  may have diverged; a clean cherry-pick still needs its tests run there.
