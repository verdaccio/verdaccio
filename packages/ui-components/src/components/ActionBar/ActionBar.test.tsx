...
describe('<ActionBar /> component', () => {
  ...
  test('when there is no action bar data', async () => {
    ...
    renderWithRouteDetail(<ActionBar packageMeta={packageMeta} />);
    await waitFor(() => {
  ...
  test('when there is a button to raw manifest', async () => {
    renderWithRouteDetail(<ActionBar packageMeta={defaultPackageMeta} showRaw={true} />);
    await waitFor(() => {
  ...
  test('should not display download tarball button', async () => {
    renderWithRouteDetail(<ActionBar packageMeta={defaultPackageMeta} showRaw={false} />);
    await waitFor(() => {
      expect(screen.queryByLabelText('Download tarball')).toBeFalsy();
    });
  });

  test.todo('when click button to download');
