...
- b2e2a5a: Avoid reporting cancelled web searches as uplink failures in error logs.
...
- 297c20e: Preserve the original cause in web search error logs when an uplink fails without an HTTP response or returns invalid JSON. DNS failures, refused or interrupted connections, timeouts, and cancellation no longer produce a secondary `TypeError` while handling the error.
...
- 3618123: fix: do not fetch client-controlled dist.tarball urls off-uplink
