...
class ProxyStorage implements IProxy {
  ...
  public fetchTarball(
    ...
    // surface the outgoing tarball request so devops can observe which host a
    // tarball is fetched from (an autogenerated proxy shows as verdaccio-<pkg>)
    this.logger.http({ url, uplink: this.uplinkName }, 'fetching tarball from @{uplink} - @{url}');
