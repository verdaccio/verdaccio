...
class Storage {
  ...
  public getTarball(name: string, filename: string) {
        ...
        const lookupFromUplinks = (info: Manifest | null): void => {
          self._syncUplinksMetadata(
            ...
            (syncErr, syncInfo: Manifest): any => {
              ...
              serveFile(distFile as DistFile, syncInfo);
            }
          );
        };
        ...
        self.localStorage.getPackageMetadataAsync(name).then(
          (info) => {
            ...
            if (_.isNil(distFile) === false) {
              ...
              debug('dist file found, using it %o', (distFile as DistFile).url);
              serveFile(distFile as DistFile, info);
            } else {
     ...
     * client's `dist.tarball` is stored verbatim, and fetching it would be
     * SSRF (and would attach uplink credentials when only one uplink is
     * configured). Uplink-synced packages may still use an autogenerated
     * proxy without credentials for CDN-hosted tarballs.
     * @param {DistFile} file distfile record resolved for the tarball
     * @param {Manifest} origin package metadata the distfile was resolved from
     */
    function serveFile(file: DistFile, origin?: Manifest | null): void {
      let uplink: any = null;
      ...
      // recorded uplink is only used when it actually serves this URL, so
      // its Authorization header is never sent to an unrelated host
      if (file.registry && self.uplinks[file.registry]) {
        const recorded = self.uplinks[file.registry];
        if (recorded.isUplinkValid(file.url)) {
          uplink = recorded;
      ...
      if (uplink == null) {
        // `_distfiles` is written when metadata is merged from an uplink.
        // Local publish only stores versions[].dist.tarball, so a version
        ...
        if (!fromDistfiles) {
          ...
          return readStream.emit('error', ErrorCode.getNotFound(API_ERROR.NO_PACKAGE));
        }
