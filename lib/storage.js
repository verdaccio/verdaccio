...
var Local     = require('./local-storage-couchdb')
...
Storage.prototype.lsearch = function(query, callback) {
	this.local.lsearch(query, function(err, r){
	  if (err) callback(err);
	  else {
	    if (r && r.length>0) {
	      var pkgs = [];
	      r.forEach(function (p){
	        var doc = p.doc;
	        if (doc && doc.package && doc.package['dist-tags'] && doc.package['dist-tags'].latest) {
	          var package;
	          if (Array.isArray(doc.package['dist-tags'].latest)) {
		          var latest = null;
		          doc.package['dist-tags'].latest.forEach (function (p){
		          	 if (!latest){
		          	    latest = p;
		          	 }
		          	 else {
		          	 	if (versionCompare(latest, p)<0)
		          	 	 latest = p;
		          	 }
		          });
		          package = doc.package.versions[latest];
	          }
	          else 
	        	  package = doc.package['dist-tags'].latest;
	          if (package._npmUser) {
		          if (!package._npmUser.name) {
		          	if (package.author)
		          		package._npmUser.name = package.author.name;
		          	else
		          	  if (package.maintainers && package.maintainers.length>0)
		          	     package._npmUser.name = package.maintainers[0].name;
		          }
		          pkgs.push(package);
	          }
	        }
...
Storage.prototype.add_tarball = function(name, filename, fullname) {
  return this.local.add_tarball(name, filename, fullname)
}
