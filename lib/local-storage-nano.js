...
var URL        = require('url')
var nano_storage = require('./local-nano')
...
Storage.prototype.add_package = function(name, info, callback) {
  console.log("local-storage-nano.add_package info", info);
...
Storage.prototype.remove_package = function(name, callback) {
  console.log("local-storage-nano.remove_package name", name);
  var self = this
  self.logger.info( { name: name }
                  , 'unpublishing @{name} (all)')

  var storage = self.storage(name)
  if (!storage) return callback( Error[404]('no such package available - TK5') )

  storage.read_json(name, function(err, data) {
    if (err) {
      if (err.code === 'ENOENT') {
        return callback( Error[404]('no such package available - TK6') )
      } else {
        return callback(err)
      }
    }
    ...
    storage.delete_json(data, function (err){
      ...
      console.log("local-storage-nano.remove_package files", files);
...
Storage.prototype._read_create_package = function(name, callback) {
  console.log("local-storage-nano._read_create_package", name)
  var self = this
  var storage = self.storage(name)
  if (!storage) {
...
Storage.prototype.update_versions = function(name, newdata, callback) {
  console.log("local-storage-nano.update_versions name", [name]);

  var self = this
  self._read_create_package(name, function(err, data) {

    console.log("local-storage return from read create package error", [err]);
...
Storage.prototype.add_tarball = function(name, filename) {
  console.log("local-storage.add_tarball", [name, filename]);
  assert(Utils.validate_name(filename))
...
Storage.prototype.get_package = function(name, options, callback) {
  console.log("local-storage.nano.get_package name", name);
  if (typeof(options) === 'function') callback = options, options = {}

  var self = this
  var storage = self.storage(name)
  if (!storage) return callback( Error[404]('no such package available -- TK1') )

  storage.read_json(name, function(err, result) {
    if (err) {
      if (err.code === 'ENOENT') {
        return callback( Error[404]('no such package available -- TK2') )
      } else {
        return callback(self._internal_error(err, info_file, 'error reading'))
      }
    }
...
Storage.prototype._each_package = function (on_package, on_end) {
  ...
  storage.list_packages (function (error, packages){
    console.log("local-storage-nano.listing packages", packages);
    async.eachSeries(packages, function (currpackage, cb) {
      console.log("local-storage-nano.listing packages storage", currpackage);
      on_package({
         name: currpackage.id,
...
Storage.prototype.update_package = function(name, updateFn, _callback) {
  var self = this
  var storage = self.storage(name)
  if (!storage) return _callback( Error[404]('no such package available - TK3') )
...
Storage.prototype.search = function(startkey, options) {
  ...
  self._each_package(function on_package(item, cb) {
      var convertedTimeToMillis = new Date(item.mtime).getTime();
      console.log("local-storage-nano.search ", [item.name, item.mtime, new Date(Number(startkey))  ]);
      ...
      //if (convertedTimeToMillis > Number(startkey)) {
        ...
        self.get_package(item.name, options, function(err, data) {
          ...
          console.log('local-storage-nano.search checking versions', [item.name, versions, latest, packageJson.versions[latest]])
...
Storage.prototype._write_package = function(name, json, callback) {
  ...
  storage.write_json(info_file, json, /*revision*/ null, callback)
}
