...
var nano      = require('nano')({
									'url' : 'https://b4e0864f-36ef-4dac-8de0-b5383f260770-bluemix:72f7442f4e6cf3448939f57b7ef519f2001d2683cd7d2979126d8c9cfa578482@b4e0864f-36ef-4dac-8de0-b5383f260770-bluemix.cloudant.com',
...
function create_json (name, value, cb) {
  console.log("local-nano.create_json name", [name, value]);
...
module.exports.update_json = function(name, value, cb) {
  console.log("local-nano.update_json name", [name, value]);
...
function delete_json (data, cb) {
  console.log("local-nano.delete_json name", [data.package.name, data['_rev']]);
  
  var sinopia = nano.db.use('sinopia');
  sinopia.destroy(data.package.name, data["_rev"] , function(err, body, header) {
    if (!err) {
      console.log('[local-nano.delete_json] ', [body]);
      return cb() ;
    }else{
    	return cb();
    }
...
function write_json (name, value, revision, cb) {
  console.log("local-nano.write_json name", [name, revision]);
  
  var sinopia = nano.db.use('sinopia');
  var payload = { "_id": value.name, "timestamp" : new Date(),  "package": value, "docType" : "package.json"};
  if(revision){
  ...
  sinopia.insert( payload , function(err, body, header) {
    if (!err) {
      console.log('[local-nano.write_json.insert] ', [body]);
      return cb() ;
    }else{
    	return cb();
    }
...
module.exports.read_json = function(name, cb) {
  console.log("local-nano.read_json name", [name]);
  var self = this;
   self.document_exists(name, function(err, headers){
		if(err && err.statusCode == 404){
		  ...
		  sinopia.get(name, function(err, body) {
		    if (err) {
		      console.log('local-nano.read_json name  error', [name, err.message]);
		      cb( NanoError('ENOENT') )
		    }else{
		      console.log('local-nano.read_json name retrieved  ', name)
...
module.exports.read_stream = function (name, stream, callback) {
	console.log("local-nano.read_stream", [name, stream, callback ]);
	...
	sinopia.attachment.get(name, name, function(err, body){
		if(err){
			...
			console.log("local-nano.read_stream name entrance body size", body && body.length)
			stream.emit('open')
...
module.exports.write_stream = function (name) {
	console.log("local-nano.write_stream", [name ]);
	...
	fs.exists(name, function(exists) {
		...
		stream.emit('success')
		console.log("local-nano.write_stream name exit", [name]);
	});
	return stream
}
...
function document_exists (name, cb) {
  console.log("local-nano.document_exists name", [name, cb]);
  var sinopia = nano.db.use('sinopia');
...
function list_packages (callback) {
  	var sinopia = nano.db.use('sinopia');
  	sinopia.view('package.json', 'list_node_packages', function(err, body) {
	  if (!err) {
	    body.rows.forEach(function(doc) {
	      console.log("local-nano.list_packages", [doc.key, doc.value]);
	    });
	  }else{
