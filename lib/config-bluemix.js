...
function envHelper(appName) {
    var env = {
        ...
        cloudant: function() {
            ...
            if (this.hasVcap()) {
                var services = this.getVcap();
                if (services && services['cloudantNoSQLDB']) {
                    // assume there is a single cloudant configured for Sinopia
                    var cred = services['cloudantNoSQLDB'][0].credentials;
                    console.log("Using VCAP_SERVICES: "+services['cloudantNoSQLDB'][0].name);
                    options = {
