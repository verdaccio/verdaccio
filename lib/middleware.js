...
module.exports.notifyWebhook = function(config, req, message) {
  if(config && config.webhook && config.webhook.url) {
    // IP not varying because we always do on jenkins
    // TODO: evaluate usefulness and remove/uncomment
    // var ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress; // get x-forwarded-for header or remoteAddress
    // ip = ip.split(',')[0]; // in this case of multiple, we care only about the first one
    var userInfo = '>' + req.remote_user.name; // format name on slack
    var postBody = JSON.stringify({ "text" : userInfo +  " " + message}) ;
    Request.post(
        config.webhook.url,
      {
        ...
        body: postBody
          ...
          // Ignore any failures to webhook
        }
    );
  }
}
