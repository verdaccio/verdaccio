...
var notifyWebhook  = Middleware.notifyWebhook
...
module.exports = function(config, auth, storage) {
  ...
  function tag_package_version(req, res, next) {
    ...
    storage.merge_tags(req.params.package, tags, function(err) {
      ...
      // TODO: evaluate usefulness and remove/uncomment
      // notifyWebhook(config, req, "added tag " + req.params.tag + " for " + req.params.package + "@" + req.body);
  ...
  app.delete('/-/package/:package/dist-tags/:tag', can('publish'), function (req, res, next) {
    ...
    storage.merge_tags(req.params.package, tags, function(err) {
      ...
      // TODO: evaluate usefulness and remove/uncomment
      // notifyWebhook(config, req, "removed tags for " + req.params.package + ": " + req.params.tag );
  ...
  app.post('/-/package/:package/dist-tags',
    ...
    storage.merge_tags(req.params.package, req.body, function(err) {
      ...
      // TODO: evaluate usefulness and remove/uncomment
      // notifyWebhook(config, req, "updated tags for " + req.params.package + ": " + JSON.stringify(req.body) );
  ...
  app.delete('/-/package/:package/dist-tags',
    ...
    storage.replace_tags(req.params.package, {}, function(err) {
      ...
      res.status(201)
      // TODO: evaluate usefulness and remove/uncomment
      // notifyWebhook(config, req, "removed all tags for " + req.params.package );
  ...
  app.put('/:package/:_rev?/:revision?', can('publish'), media('application/json'), expect_json, function(req, res, next) {
    ...
    function after_change(err, ok_message) {
      ...
      create_tarball(Path.basename(t1), t1, metadata._attachments[t1], function(err) {
        if (err) return next(err)
        ...
        create_version(t2, metadata.versions[t2], function(err) {
          ...
          add_tags(metadata['dist-tags'], function(err) {
            ...
            var webHookMessage = ":newspaper: : `" + metadata.name + "@" + t2 + '`';

            // TODO: evaluate usefulness and remove/uncomment
            // if (metadata.versions[t2].repository && metadata.versions[t2].repository.url) {
            //   webHookMessage = webHookMessage + " repository url: " + metadata.versions[t2].repository.url;
            //   if (metadata.versions[t2].gitHead) {
            //     webHookMessage = webHookMessage + " gitHead:" + metadata.versions[t2].gitHead;
            //   }

            // }
            notifyWebhook(config, req, webHookMessage);
            return next({ ok: ok_message })
          })
        })
      })
    ...
    function create_tarball(filename, fullname, data, cb) {
      var stream = storage.add_tarball(name, filename, fullname)
      stream.on('error', function(err) {
        cb(err)
      })
  ...
  app.delete('/:package/-rev/*', can('publish'), function(req, res, next) {
    storage.remove_package(req.params.package, function(err) {
      ...
      notifyWebhook(config, req,  "unpublished entire package :bangbang: : `" + req.params.package + '`');
  ...
  app.delete('/:package/-/:filename/-rev/:revision', can('publish'), function(req, res, next) {
    storage.remove_tarball(req.params.package, req.params.filename, req.params.revision, function(err) {
      if (err) return next(err)
      res.status(201)
      notifyWebhook(config, req, "unpublished :warning: : `" + req.params.filename.replace(".tgz","") + '`');
  ...
  app.put('/:package/:version/-tag/:tag', can('publish'), media('application/json'), expect_json, function(req, res, next) {
    ...
    storage.add_version(name, version, req.body, tag, function(err) {
      ...
      // TODO: evaluate usefulness and remove/uncomment
      // notifyWebhook(config, req, "added tag " + tag + " for " + name + "@" + req.params.version);
